...
public class MainActivity extends Activity implements OnCheckedChangeListener,OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ListAdapter listadapter = new ListAdapter(this.getApplicationContext(),apps, this);
        ...
        totalbytes.setText("Total: " + conversor.bytesBinaryConvertion(NetSniffer.getTotalRxBytes()+NetSniffer.getTotalTxBytes()));
	...
	public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
		//idea: if anyone if checked a Button appears at the end, but not working with the first item checked
		
		nchecked = isChecked ? nchecked+1 : nchecked-1;
			
		Button b = (Button) this.findViewById(R.id.footer);
		b.setText(getResources().getString(R.string.task_killer)+" "+String.valueOf(nchecked));
		
		/*if (nchecked > 0  && (b.getVisibility()==Button.GONE))
			b.setVisibility(Button.VISIBLE);
		else if (nchecked == 0  && (b.getVisibility()==Button.VISIBLE))
	...
	public static String[] getRunningAppNames(Context context){
	    ...
	    List<RunningAppProcessInfo> runningProcess = amanager.getRunningAppProcesses();
	...
	public void onClick(View v) {
		...
		// kill selected apps. keep selected pkg names in a list and then go through the list
